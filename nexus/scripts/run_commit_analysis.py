...
def commit_subject(oneline: str, sha: str) -> str:
    parts = oneline.split(maxsplit=1)
    return parts[1] if len(parts) == 2 else sha
...
def local_analysis(commits: list[dict[str, Any]]) -> str:
    ...
    for c in commits:
        sha = c["sha"]
        subject = commit_subject(c["oneline"], sha)
        detail = c["details"]
...
def main() -> None:
    ...
    commit_list = "\n".join(
        f"- `{c['sha'][:7]}` {commit_subject(c['oneline'], c['sha'])}" for c in commit_details
    ) or "- (none)"
